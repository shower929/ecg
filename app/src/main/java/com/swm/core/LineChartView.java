...
public class LineChartView extends SurfaceView {
    ...
    private PlottingWorker mEcgPlotting;
    ...
    private class PlottingWorker extends Thread {
        int x1=0;
        ...
        @Override
        public void run() {
            Log.v(LOG_TAG, "Plotting worker is running");
            super.run();
            try {

                for (;;) {
                    if (!mRendering) {
                        Log.v(LOG_TAG, "Stop rendering");
                        return;
                    }

                    synchronized (mLineBuffer) {
                        int len = mLineBuffer.size();

                        for(int i = 0; i < len; i++) {
                            drawLine(mLineBuffer.get(i));
                        }
                    }

                    Thread.sleep(SwmCore.CLOCK);
                }
            } catch (InterruptedException e) {
                Log.v(LOG_TAG, "Plotting stop");
            }
        }
        ...
        private void drawLine (Float value) {
            ...
            mBackgroundDrawing.drawLine(x1, y1, x2, y2, mPaint);
            x1 = x2;
            y1 = y2;
    ...
    private void init() {
        ...
        mPaint.setColor(getResources().getColor(R.color.swm_ecg_color));
        mPaint.setStrokeWidth(LINE_SIZE * mDensity);

        mClearPaint = new Paint();
        mClearPaint.setColor(Color.BLACK);

        mLineBuffer = new ArrayList<>();

        mProfilingHandler = new Handler();

    }

    public synchronized void offerValue(float value) {
        if (mRendering)
            synchronized (mLineBuffer) {
                mLineBuffer.add(Float.valueOf(value));
            }
    ...
    private synchronized void startDrawing() {
        ...
        mRendering = true;
        mEcgPlotting = new PlottingWorker();
        mEcgPlotting.start();
        mProfiling = new Runnable() {
            @Override
            public void run() {
                Log.v(LOG_TAG, "ECG rendering buffer: " + mLineBuffer.size());
